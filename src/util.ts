...
export async function addLiquidity(
  ...
  console.log(
    `addLiquidity: ${poolKeys.id.toBase58()}, base amount: ${amount.toFixed()}, quote amount: ${anotherAmount.toFixed()}`
  );
...
export async function removeLiquidity(
  ...
  const lpToken = tokenAccounts.find(
    (t) => t.accountInfo.mint.toBase58() === poolKeys.lpMint.toBase58()
  );

  if (lpToken) {
    const ratio =
      parseFloat(lpToken.accountInfo.amount.toString()) /
      parseFloat(poolInfo.lpSupply.toString());
    console.log(
      `base amount: ${
        (poolInfo.baseReserve.toNumber() * ratio) / 10 ** poolInfo.baseDecimals
      }, quote amount: ${
        (poolInfo.quoteReserve.toNumber() * ratio) /
        10 ** poolInfo.quoteDecimals
      } `
    );

    const amountIn = new TokenAmount(
      new Token(poolKeys.lpMint, poolInfo.lpDecimals),
      lpToken.accountInfo.amount.toNumber()
    );
...
export async function routeSwap(
  ...
  const owner = ownerKeypair.publicKey;
  const fromPoolInfo = await Liquidity.fetchInfo({
    connection,
    poolKeys: fromPoolKeys,
  });
  const toPoolInfo = await Liquidity.fetchInfo({
    connection,
    poolKeys: toPoolKeys,
  });
  const amountIn = new TokenAmount(
    new Token(fromPoolKeys.baseMint, fromPoolInfo.baseDecimals),
    1,
    false
  );
  ...
  const { amountOut, minAmountOut, executionPrice, priceImpact, fee } =
    Route.computeAmountOut({
      fromPoolKeys,
      toPoolKeys,
      fromPoolInfo,
      toPoolInfo,
  ...
  // @ts-ignore
  console.log(
    `route swap: ${fromPoolKeys.id.toBase58()}, amountIn: ${amountIn.toFixed()}, amountOut: ${amountOut.toFixed()}, executionPrice: ${executionPrice!.toFixed()}`
  );

  const { setupTransaction, swapTransaction } = await Route.makeSwapTransaction(
    {
      connection,
      fromPoolKeys,
      toPoolKeys,
  ...
  if (setupTransaction) {
    await sendTx(connection, setupTransaction.transaction, [
      ownerKeypair,
      ...setupTransaction.signers,
    ]);
  }

  if (swapTransaction) {
    await sendTx(connection, swapTransaction.transaction, [
      ownerKeypair,
...
export async function tradeSwap(
  connection: Connection,
  tokenInMint: PublicKey,
  tokenOutMint: PublicKey,
  relatedPoolKeys: LiquidityPoolKeys[],
  ownerKeypair: Keypair,
  tokenAccounts: TokenAccount[]
) {
  ...
  const pools = await Promise.all(
    relatedPoolKeys.map(async (poolKeys) => {
      const poolInfo = await Liquidity.fetchInfo({ connection, poolKeys });
      return {
        poolKeys,
        poolInfo,
  ...
  console.log(
    `trade swap: amountIn: ${amountIn.toFixed()}, amountOut: ${amountOut.toFixed()}, executionPrice: ${executionPrice!.toFixed()}, ${routeType}`
  );
  ...
  if (tradeTransaction) {
    await sendTx(connection, tradeTransaction.transaction, [
      ownerKeypair,
      ...tradeTransaction.signers,
    ]);
  }
